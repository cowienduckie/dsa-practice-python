...
class BinaryIndexedTree:
    """
    Version 1-indexed of Binary Indexed Tree (BIT), also known as Fenwick Tree.
    """
    ...
    def __init__(self, size):
        ...
        self.size = size
        self.tree = [0] * (size + 1)  # 1-indexed BIT
    ...
    def fast_load_array(self, arr: List[int]) -> None:
        ...
        for i in range(len(arr)):
            self.tree[i + 1] += arr[i]
            if (j := i + 1 + (i + 1 & -(i + 1))) <= self.size:
                self.tree[j] += self.tree[i + 1]

    def add(self, index: int, delta: int) -> None:
        """
        Add a delta to the BIT at a given index (1-indexed)
    ...
    def sum(self, index: int) -> int:
        """
        Get the sum of the BIT from index 1 to index (inclusive)

        :param int index: The index to get the sum from
        :return: The sum of the BIT from index 1 to index (inclusive)
        """
        result = 0
        while index > 0:
    ...
    def range_sum(self, start: int, end: int) -> int:
        """
        Get the sum of the BIT from index start to index end (inclusive)

        :param int start: The start index
        :param int end: The end index
        :return: The sum of the BIT from index start to index end (inclusive)
        """
